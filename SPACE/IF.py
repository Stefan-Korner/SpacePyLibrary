...
# ================================
# Variable packet definition level
# ================================
...
class VPslotDef(object):
  ...
  # ---------------------------------------------------------------------------
  def __str__(self, indent="VPslotDef"):
    """string representation"""
    return self.childDef.__str__(indent + "." + self.slotName)

# =============================================================================
class VPstructDef(object):
  ...
  def __init__(self, structName, sortedSlotDefs):
    ...
    for slotDef in self.slotDefs:
      ...
      slotPos += 1
  # ---------------------------------------------------------------------------
  def __str__(self, indent="VPstructDef"):
    """string representation"""
    if self.structName != "":
...
class VPlistDef(object):
  ...
  # ---------------------------------------------------------------------------
  def __str__(self, indent="VPlistDef"):
    """string representation"""
...
# ==============================
# Variable packet instance level
# ==============================

# =============================================================================
class VPparam(object):
  ...
  # ---------------------------------------------------------------------------
  def __str__(self, indent="VPparam"):
    """string representation"""
...
class VPslot(object):
  ...
  def __init__(self, slotDef=""):
    ...
    if childType == VPparamDef:
      self.child = VPparam(paramDef=childDef)
    elif childType == VPlistDef:
      self.child = VPlist(listDef=childDef)
    else:
      print("error: child type " + childType + " not supported")
      sys.exit(-1)
  # ---------------------------------------------------------------------------
  def __str__(self, indent="VPslot"):
    """string representation"""
...
# =============================================================================
class VPstruct(object):
  """Struct in a variable packet"""
  # ---------------------------------------------------------------------------
  def __init__(self, structDef):
    self.structDef = structDef
    self.slots = tuple(VPslot(slotDef) for slotDef in structDef.slotDefs)
  # ---------------------------------------------------------------------------
  def __str__(self, indent="VPstruct"):
    """string representation"""
    retStr = ""
...
class VPlist(object):
  ...
  def __init__(self, listDef):
    ...
    self.entries = [VPstruct(entryDef) for _ in range(0, length)]
  # ---------------------------------------------------------------------------
  def __str__(self, indent="VPlist"):
    """string representation"""
  ...
  def setLen(self, length):
    ...
    if length < oldLen:
      ...
      while len(self.entries) < length:
        self.entries.append(VPstruct(entryDef))
  # ---------------------------------------------------------------------------
